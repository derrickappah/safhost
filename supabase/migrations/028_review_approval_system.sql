...
-- Handle existing reviews with ratings < 2 and no comment
-- Set a default comment for any existing reviews that would violate the constraint
UPDATE reviews 
SET comment = 'No comment provided'
WHERE rating < 2 AND (comment IS NULL OR comment = '');
